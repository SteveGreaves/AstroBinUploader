...
A FITS header file generated by N.I.N.A is quite rich, an example is given below:
...
The header file parameters used by the code are:
...
The 'Filter.csv' file defines the mapping from the filter name to AstroBin filter code. Below is an example showing my filter set of Astronomik filters and their AstroBin codes:
...
For example, consider a Ha filter: a [2-inch H-alpha CCD 6nm filter from Astronomik](https://app.astrobin.com/equipment/explorer/filter/4663/astronomik-h-alpha-ccd-6nm-2). By using [AstroBin's filter explorer](https://app.astrobin.com/equipment/explorer/filter?page=1) to navigate to this filter's page the URL is found to be :
