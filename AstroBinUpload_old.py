...
def process_fits_data(directory, bortle, mean_sqm, mean_fwhm):
    """
    ...
    try:
        ...
        output_csv = os.path.basename(directory) + " acquisition.csv"
...
def main():
    ...
    """
    try:
        if not os.path.isfile('filters.csv'):
            raise ValueError("Filter file not found.\nPlease create filters.csv in the local directory and run again.")
        
        directory = input("Enter the image directory: ")
        if not os.path.isdir(directory):
            raise ValueError("Directory not found.\nPlease enter a valid image directory.")
        ...
        process_fits_data(directory, bortle, mean_sqm, mean_fwhm)
