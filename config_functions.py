...
def cast_value(value: Any, logger: Optional[logging.Logger] = None) -> Any:
    ...
    try:
        ...
        if isinstance(value, list):
            logger.info("Casting list value") if logger else None
        ...
        if isinstance(value, str) and '.' in value:
            try:
                ...
                logger.debug(f"Cast {value} to float: {cast_result}") if logger else None
                return cast_result
            except ValueError:
                logger.debug(f"Failed to cast {value} to float, returning as string") if logger else None
                return value
        # Attempt to cast to integer for numeric strings
        try:
            cast_result = int(value)
            logger.debug(f"Cast {value} to int: {cast_result}") if logger else None
            return cast_result
        except (ValueError, TypeError):
            # Return original value if casting fails
            logger.debug(f"Returning {value} as is") if logger else None
            return value
