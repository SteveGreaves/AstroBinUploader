...
--------------------------------------------------------------------------------------------
version 1.0.4
4th December 2023 changes

1. Corrected issue with Bortle and SQM values not being updated correctly
2. Corrected issue with Keywords from .XISF files not being read correctly 
3. Improved code to correct file data reading and saving logic
4. No longer requred to manually create configutration csv files. 
...
5. Runtime option to stop program if new csv files are created to edit them.
6. Corrected program logic related to import, access and storage of external parmeters.
7. Refactored code to improve readability
...
def read_or_create_csv(dictionaries):
    ...
    It also strips whitespaces from string columns in the DataFrame.
...
def extract_headers(directories, default_values):
    """
    Extracts and processes image headers from files within specified directories.

    This function iterates through given directories, extracting headers from FITS and XISF files.
    It then synchronizes these headers with default values provided in 'default_values'.
    The function returns a pandas DataFrame containing the processed headers for all files.

    Parameters:
    - directories (list): A list of directory paths where image files are located.
    - default_values (pandas.DataFrame): A DataFrame containing default header values.

    Returns:
    - pandas.DataFrame: A DataFrame of processed headers, with one row per image file.
    """
    #Definitions of functions used only in extract_headers
    ...
    # Set 'FITSKeyword' as the index in default_values DataFrame if not already set
    try:
    ...
    swcreate = headers[-1].get('SWCREATE', 'unknown package') if headers else 'unknown package'
    print(f"\nImages captured by {swcreate}")

    return pd.DataFrame(headers)  # Convert list of headers to DataFrame


def summarize_session(df):    
    """
    Summarizes an astronomical observation session from a DataFrame.

    This function analyzes a DataFrame containing observation session data and produces a summary string.
    It categorizes data based on 'IMAGETYP' (e.g., LIGHT, FLAT, BIAS, DARK), and summarizes the
    number of frames and total exposure time for each category. The summary includes details like
    filter types, gain values, and formatted exposure times.

    Parameters:
    - df (pandas.DataFrame): A DataFrame containing session data with columns like 'IMAGETYP', 'FILTER', 'EXPOSURE', 'GAIN'.

    Returns:
    - str: A formatted summary string of the observation session.
    """

    #Definitions of functions used only in summarize_session
...
def create_lights_df(df: pd.DataFrame)-> pd.DataFrame:
    ...
    # Convert the 'DATE-LOC' column to datetime format and extract the date
    light_df['DATE-LOC'] = pd.to_datetime(light_df['DATE-LOC']).dt.date

    # Return the DataFrame with 'LIGHT' type data rounded to 2 decimal places
    return pd.DataFrame(light_df).round(2)


def calculate_auxiliary_parameters(df, defaults_df, secret_df, sites_df):
    """
    Calculates auxiliary parameters for astronomical data and updates the DataFrame.

    This function processes a DataFrame containing astronomical observation data and calculates
    various auxiliary parameters like Bortle scale classification, Sky Quality Meter (SQM) readings,
    Half Flux Radius (HFR), Image Scale (IMSCALE), and Full Width at Half Maximum (FWHM).
    The Bortle and SQM values are obtained either from a local data source (sites_df) or via an API request,
    depending on whether the site data is already available.

    Parameters:
    - df (pandas.DataFrame): The main DataFrame containing observation data.
    - defaults_df (pandas.DataFrame): A DataFrame with default values for certain parameters.
    - secret_df (pandas.DataFrame): A DataFrame containing secret data like API keys.
    - sites_df (pandas.DataFrame): A DataFrame containing site data including Bortle and SQM readings.

    Returns:
    - pandas.DataFrame: The updated DataFrame with new auxiliary parameters.
    """

    #Definitions of functions only used in calculate_auxiliary_parameters
    ...
    def get_bortle_sqm(lat, lon, secret_df):
        """
        Fetches Bortle and SQM values for given latitude and longitude, using an API.

        This function makes an API request to obtain Bortle scale classification and SQM values for
        specified coordinates. It handles API authentication, request formation, and response processing.
        In case of API issues or invalid data, it handles errors gracefully.
        ...
        Returns:
        - tuple: Contains Bortle class, SQM value, error message (if any), and validity flags for API key and endpoint.
        """
        # Function implementation
   ...
   #main code for calculate_auxiliary_parameters
    ...
    #print('Sites file is empty: ', empty_sites)
    ...
    for index, row in df.iterrows():
        ...
        site_data = ((sites_df['Latitude'] == lat) & (sites_df['Longitude'] == lon)).any()

        #check lat and long dont exist in processd sites
        coordinates_processed = (lat, lon) in processed_sites
        ...
        if not site_data:
            if valid_api_key:
                ...
                bortle, sqm, api_response_text, valid_api_key, valid_api_endpoint = get_bortle_sqm(lat, lon,secret_df)
        ...
        if not (lat,lon) in processed_sites:
...
def create_astrobin_output(df, filter_df):
   ...
   # filter_df.set_index('filter', inplace=True)

    # Creating a Series to map filter names to codes
    ...
    def update_filter(filter_value):
        """
        Maps a filter name to its corresponding code based on the 'filter_df' DataFrame.

        If the filter name exists in 'filter_df', this function returns the associated code.
        ...
        #check code is a four digit integer, if it is assume valid code
        ...
        try:
            ...
            code = None
        if isinstance(code, int) and 1000 <= code <= 9999:
            return code
        else:
            # Print an error message if the code is not found for a filter
            print(f"\nError: for filter {filter_value}: no code found. Enter a valid code in filters.csv file or input the code in the astrobin upload file.")
            return f"{filter_value}: no code found"

    # Apply the update_filter function to the 'filter' column
    df['filter'] = df['filter'].apply(update_filter) 
    ...
    return df[column_order].round(2)
